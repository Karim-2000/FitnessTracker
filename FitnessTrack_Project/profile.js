...
function loadWeightProgress(userId) {
    Promise.all([
        ...
        firebase.database().ref(`measurements/${userId}`).orderByKey().limitToLast(1).once('value')
    ]).then(([goalsSnapshot, measurementsSnapshot]) => {
        ...
        if (goals && measurements) {
            ...
            const currentWeight = Object.values(measurements)[0].weight;
            ...
            document.getElementById('weightProgress').textContent = `${Math.round(progress)}%`;
